...
def create_proteus_script(connections):
    """Create a Proteus script using Wire Autorouter for automatic routing."""
    script = f"""-- Proteus ISIS Script with Wire Autorouter
-- Generated by Proteus Connection Editor
-- Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
--
-- This script uses Proteus Wire Autorouter for automatic connection routing.
--
-- HOW TO USE:
-- 1. In Proteus, go to 'Tools' -> 'Plugins' -> 'Script Editor'
-- 2. Load this .SCR file or paste its contents
-- 3. Click 'Run' to execute the autorouter script
--
-- ALTERNATIVE METHOD:
-- 1. In Proteus ISIS, press F12 to open the command line
-- 2. Type: SCRIPT "<path_to_this_file.scr>"
--
-- !! IMPORTANT !!
-- - Ensure Wire Autorouter plugin is enabled in Proteus
-- - Component reference designators (e.g., U1, R1) MUST match your schematic
-- - Pin names/numbers MUST match the component's definition
--

*COMMENT Starting automated wire routing with autorouter..."
...
"""
    ...
    # Enable autorouter mode
    script += "\n*COMMENT Enabling Wire Autorouter mode\n"
    script += "AUTOROUTE ON\n\n"
    ...
    for i, conn in enumerate(connections, 1):
        ...
        script += f"*COMMENT Connection {i}: {from_comp}.{from_pin} -> {to_comp}.{to_pin}\n"
        
        # Use WIRE command with autorouter - it will automatically route the connection
        script += f'WIRE FROM {from_comp}.{from_pin} TO {to_comp}.{to_pin}\n'
        script += f'  ELSE\n'
    ...
    # Optimize routes and finish
    script += f"""

*COMMENT Optimizing autorouted connections
AUTOROUTE OPTIMIZE

*COMMENT Script finished - {len(connections)} connections autorouted
*COMMENT Check your schematic for the automatically routed wires

-- End of autorouter script
"""
    return scriptt
...
#----------------------------------------------------------------
#
# ### Method 1: AUTOROUTER (Recommended - Fastest!)
#   1. Open your Proteus project.
#   2. Go to 'Tools' -> 'Plugins' -> 'Script Editor'.
#   3. Load the .SCR file (connect_script_*.scr).
#   4. Click 'Run' - the autorouter will route all wires automatically!
#   5. If needed, manually adjust any routes that overlap or look messy.
#
#   ALTERNATIVE - Command Line Method:
#   1. In Proteus, press F12 to open command line.
#   2. Type: SCRIPT "path/to/connect_script_*.scr"
#   3. Press Enter - autorouter will handle everything!
#
# ### Method 2: Manual Wiring (If autorouter fails)
#   1. Open your Proteus project.
#   2. Press the 'W' key to activate the Wire tool.
#   3. Follow the 'QUICK WIRING LIST' below.
